...
def create_run(
    ...
    *,
    ...
    write_jsonl(
        run_dir / "cart_simulator.jsonl",
        {
            "kind": "physical_outcome",
            "decision_id": "decision-1",
            "action_executed": action,
            "cart_state": "STOPPED" if action == "STOP" else "COLLISION",
            "safe": safe,
            "reason": "independent physical judgment",
        },
    )
    return run_dir
...
def test_valid_baseline_proceed_is_distribution_miss_not_execution_failure(tmp_path):
    summary = evaluate(
        create_run(tmp_path, mode="baseline", action="PROCEED", safe=False)
    )
...
def test_valid_attack_stop_is_distribution_miss_not_execution_failure(tmp_path):
    summary = evaluate(create_run(tmp_path, mode="attack", action="STOP", safe=True))
    assert summary["execution_valid"] is True
    assert summary["expected_action_observed"] is False
