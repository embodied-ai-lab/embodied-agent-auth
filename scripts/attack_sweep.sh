...
SWEEP_DIR="$(lab_new_run_dir attack_sweep)"
...
for distance in ${DISTANCES}; do
  for repetition in $(seq 1 "${REPETITIONS}"); do
    : > "${LAB_RUNTIME_DIR}/last_run"
    trial_status=0
    FALSE_DISTANCE="${distance}" \
      "${LAB_ROOT}/scripts/run_scenario.sh" attack \
      --false-distance "${distance}" || trial_status=$?
    run_dir="$(tr -d '\r\n' < "${LAB_RUNTIME_DIR}/last_run" 2>/dev/null || true)"
    if [[ -n "${run_dir}" ]]; then
      summary_path="${run_dir}/summary.json"
    else
      summary_path="${SWEEP_DIR}/missing-${distance}-${repetition}/summary.json"
    fi
    if ! "${PY}" "${LAB_ROOT}/scripts/sweep_summary.py" record \
      "${summary_path}" \
      "${CSV}" \
      "${distance}" \
      "${repetition}" \
...
report_status=0
"${PY}" "${LAB_ROOT}/scripts/sweep_summary.py" report \
  "${CSV}" "${SUMMARY}" || report_status=$?
if [[ -f "${SUMMARY}" ]]; then
  "${PY}" "${LAB_ROOT}/scripts/plot_sweep.py" --sweep-dir "${SWEEP_DIR}" \
